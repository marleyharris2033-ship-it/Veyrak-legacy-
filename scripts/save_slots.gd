...
var settings = {"muted": false, "volume": 0.8, "music": 0.8, "sfx": 0.8}
...
func _ready() -> void:
	...
	if config.load("user://settings.cfg") == OK:
		...
		settings.music = clampf(float(config.get_value("audio", "music", 0.8)), 0, 1)
		settings.sfx = clampf(float(config.get_value("audio", "sfx", 0.8)), 0, 1)
...
func delete_slot(slot: int) -> Error:
	if slot < 1 or slot > SLOT_COUNT: return ERR_INVALID_PARAMETER
	var path = path_for(slot)
	if FileAccess.file_exists(path): return DirAccess.remove_absolute(path)
	return OK
...
func apply_audio() -> void:
	for pair in [["Music","music"],["SFX","sfx"]]:
		...
		if index >= 0:
			AudioServer.set_bus_volume_db(index,linear_to_db(maxf(.001,settings[pair[1]])))
			AudioServer.set_bus_mute(index,settings[pair[1]] == 0)
	AudioServer.set_bus_mute(0, settings.muted or settings.volume == 0)
...
func save_settings() -> Error:
	...
	config.set_value("audio", "music", settings.music)
	config.set_value("audio", "sfx", settings.sfx)
