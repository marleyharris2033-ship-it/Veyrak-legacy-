...
func run() -> void:
	...
	for id in VeyrakHeroes.IDS:
		...
		for dimensions in [Vector2i(390,844),Vector2i(844,390)]:
			...
			check(game.hud.quick_slots.size() == 8,"Mixed hotbar has eight slots")
			check(game.hud.hotbar_panel.get_global_rect().end.x <= root.size.x,"Hotbar fits viewport")
			check(game.hud.hotbar_panel.get_global_rect().end.y <= root.size.y,"Hotbar fits viewport vertically")
